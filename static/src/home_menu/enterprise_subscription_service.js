...
export class CustomSubscriptionManager {
...
class CustomExpiredSubscriptionBlockUI extends Component {
    setup() {
        this.subscription = useState(useService("enterprise_subscription"));
        console.log('adsadadsadadasd')
    }
}
CustomExpiredSubscriptionBlockUI.props = {};
CustomExpiredSubscriptionBlockUI.template = xml`
...
`;
// CustomExpiredSubscriptionBlockUI.components = { ExpirationPanel };

export const CustomEnterpriseSubscriptionService = {
    name: "enterprise_subscription",
    dependencies: ["orm", "rpc", "notification"],
    start(env, { rpc, orm, notification }) {
        registry
            .category("main_components")
            .add("expired_subscription_block_ui", { Component: CustomExpiredSubscriptionBlockUI });
        return new CustomSubscriptionManager(env, { rpc, orm, notification });
    },
};

registry.category("services").remove("enterprise_subscription");
registry.category("services").add("enterprise_subscription", CustomEnterpriseSubscriptionService);
